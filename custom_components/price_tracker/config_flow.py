...
from custom_components.price_tracker.utils import findItem, findValueOrDefault, md5, removeItem
from .const import CONF_DEVICE, CONF_GS_NAVER_LOGIN_CODE, CONF_GS_NAVER_LOGIN_FLOW_2_URL, CONF_GS_STORE_CODE, CONF_ITEM_DEVICE_CODE, \
    ...
    CONF_OPTION_DATA_SCHEMA, CONF_ITEM_URL, CONF_ITEM_MANAGEMENT_CATEGORY, CONF_OPTION_DEVICES
...
class PriceTrackerConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_gs_login(self, user_input=None):
        ...
        if user_input is not None:
            ...
            devices = {**response, 'device_id': user_input[CONF_ITEM_DEVICE_CODE], 'gs_device_id': device_id, 'store': user_input[CONF_GS_STORE_CODE]}
            ...
            if (
                ...
                self._abort_if_unique_id_configured(updates={CONF_TYPE: type, CONF_DEVICE: entry.data[CONF_DEVICE] + [devices]})
            else:
                self._abort_if_unique_id_configured()
        ...
        if CONF_DEVICE in self.config_entry.data and len(self.config_entry.data[CONF_DEVICE]) > 0:
            if user_input is None:
                ...
                for d in dr.async_entries_for_config_entry(dr.async_get(self.hass), self.config_entry.entry_id):
                    device_entitites.append(d.id)

                return self.async_show_form(
                    step_id="init", data_schema=vol.Schema({
                        vol.Optional(CONF_OPTION_DEVICES): selector.DeviceSelector(
                    selector.DeviceSelectorConfig(filter=selector.DeviceFilterSelectorConfig(integration=DOMAIN))),
            }), errors=errors)

        if user_input is not None:
            if not errors:
                if user_input.get(CONF_OPTION_SELECT) == CONF_OPTION_MODIFY:
                    return await self.async_step_select(user_input={CONF_OPTION_DEVICES: user_input.get(CONF_OPTION_DEVICES)})
    ...
    async def async_step_select(self, user_input: dict = None):
        ...
        for e in entities:
            ...
            if CONF_OPTION_DEVICES in user_input and user_input[CONF_OPTION_DEVICES] is not None:
                if entity.device_id != user_input[CONF_OPTION_DEVICES]:
                    continue
