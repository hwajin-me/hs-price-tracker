...
class GsTheFreshLogin:
    ...
    async def naver_login(self, code: str, device_id: str) -> dict[str, Any]:
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(verify_ssl=False)) as session:
            async with session.get(url=CONF_GS_NAVER_LOGIN_FLOW_2_URL.format(code),
                                   headers={**REQUEST_DEFAULT_HEADERS, **_REQUEST_HEADERS}) as response:
                ...
                async with session.post(url=CONF_GS_NAVER_LOGIN_FLOW_3_URL,
                                        headers={**REQUEST_DEFAULT_HEADERS, **_REQUEST_HEADERS, 'device_id': device_id,
                                                 'appinfo_device_id': device_id,
                                                 'Authorization': 'Bearer {}'.format(temp_access_token),
                                                 'content-type': 'application/json'},
                                        json={'socialType': 'naver'}) as login:
                    if login.status != 200:
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(verify_ssl=False)) as session:
            async with session.post(url=_LOGIN_URL, json={
                ...
                'password': hash_password
                        ...
                        'appinfo_device_id': device_id}) as response:
                if response.status != 200:
                    raise Exception('')
...
class GsTheFreshDevice(Device):
    ...
    def __init__(self, device_id: str, gs_device_id: str, access_token: str, refresh_token: str, name: str, number: str,
                 store: str):
