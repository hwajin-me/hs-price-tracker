            ...
            if response is not None:
                if response.status_code == 200:
                    ...
                    if soup is not None:
                        ...
                        page_atf = find_item(j['props']['pageProps']['pageList'], 'page', 'PAGE_ATF')
                        if page_atf is None:
                            raise Exception("Coupang Parse Error (No ATF) - {}".format(data))
                        page_atf = page_atf['widgetList']

                        name = find_item(page_atf, 'viewType', 'MWEB_PRODUCT_DETAIL_PRODUCT_INFO')['data']['title']
                        price = \
                            find_item(page_atf, 'viewType', 'MWEB_PRODUCT_DETAIL_ATF_PRICE_INFO')['data']['finalPrice'][
                                'price']
                        price_info = find_item(page_atf, 'viewType', 'MWEB_PRODUCT_DETAIL_ATF_PRICE_INFO')['data']
                        ...
                        description = ''
                        image = \
                            find_item(page_atf, 'viewType', 'MWEB_PRODUCT_DETAIL_ITEM_THUMBNAILS')['data']['medias'][0][
                                'detail']
                        category = ">".join(str(t['name']) for t in
                                            find_item(page_atf, 'viewType',
                        ...
                        inventory = find_item(page_atf, 'viewType', 'MWEB_PRODUCT_DETAIL_ATF_QUANTITY')[
                            'data'] if find_item(page_atf, 'viewType',
